...
const usePropMap = (props, ctx: SetupContext) => {
  ...
  const useComputed = (
    ...
    dependPropKeys: string | string[],
    compute: (...args: any[]) => any = (v) => v,
  ) => {
    let oldValues: any[] = [];
    let oldResult = null;
    return computed(() => {
...
function setMergeMapSymbol(mergeMap, momentRefMap) {
  mergeMap[$deletePropList] = mergeArray(
    momentRefMap[$deletePropList],
    mergeMap[$deletePropList] || [],
  );

  if (mergeMap[$render] && typeof mergeMap[$render] === 'function') {
    mergeMap[$render] = momentRefMap[$render].concat([
      mergeMap[$render],
    ]) as any;
  } else {
